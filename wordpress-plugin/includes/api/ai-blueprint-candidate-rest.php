...
add_action( 'rest_api_init', 'factory_register_ai_blueprint_candidate_rest_routes' );

function factory_register_ai_blueprint_candidate_rest_routes(): void {
	register_rest_route(
		'factory/v1',
		'/ai/blueprint-candidate',
		[
			'methods'             => 'POST',
			'callback'            => 'factory_rest_ai_blueprint_candidate',
			'permission_callback' => 'factory_rest_require_manage_options',
		]
	);
}

function factory_rest_ai_blueprint_candidate( WP_REST_Request $request ): WP_REST_Response {
	...
	$response = factory_ai_build_blueprint_candidate(
		[
			'prompt'    => $prompt,
			'site_plan' => $site_plan,
