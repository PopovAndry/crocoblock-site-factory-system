...
add_action( 'rest_api_init', 'factory_register_ai_generate_preflight_rest_routes' );

function factory_register_ai_generate_preflight_rest_routes(): void {
	register_rest_route(
		'factory/v1',
		'/ai/generate-preflight',
		[
			'methods'             => 'POST',
			'callback'            => 'factory_rest_ai_generate_preflight',
			'permission_callback' => 'factory_rest_require_manage_options',
		]
	);
}

function factory_rest_ai_generate_preflight( WP_REST_Request $request ): WP_REST_Response {
	...
	$response = factory_ai_build_generate_preflight(
		[
			'prompt'              => $prompt,
			'site_plan'           => $site_plan,
