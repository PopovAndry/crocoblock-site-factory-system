...
add_action( 'rest_api_init', 'factory_register_ai_preview_diff_rest_routes' );

function factory_register_ai_preview_diff_rest_routes(): void {
	register_rest_route(
		'factory/v1',
		'/ai/preview-diff',
		[
			'methods'             => 'POST',
			'callback'            => 'factory_rest_ai_preview_diff',
			'permission_callback' => 'factory_rest_require_manage_options',
		]
	);
}

function factory_rest_ai_preview_diff( WP_REST_Request $request ): WP_REST_Response {
	...
	$response = factory_ai_build_preview_diff(
		[
			'prompt'              => $prompt,
			'site_plan'           => $site_plan,
