...
class Factory_Render_Adapter {
					...
					<header style="position: relative; margin-bottom: 28px; border: 1px solid <?php echo esc_attr( $style_tokens['border'] ); ?>; border-radius: 24px; background: <?php echo esc_attr( $style_tokens['background'] ); ?>; overflow: hidden;">
						...
						<div style="position: relative; padding: clamp(24px, 4vw, 38px);">
							...
							<p style="max-width: 720px; color: <?php echo esc_attr( $style_tokens['muted'] ); ?>; font-size: 16px; line-height: 1.62; margin: 0;">
								Explore apartments, houses, and commercial spaces across Kyiv with clear filters and direct viewing requests.
							</p>
						</div>
					</header>
	...
	public function render_generated_footer( array $blueprint ): string {
		...
		$html .= '<p style="color: rgba(255,255,255,0.88); font-size: 14px; line-height: 1.6; margin: 0;">' . esc_html( $footer_description ) . '</p>';
		$html .= '</div>';
	...
	private function render_home_benefits_section( array $blueprint ): string {
		...
		$border         = $style_tokens['border'];
		$benefits       = [
			[ 'icon' => '01', 'title' => 'Apartments, houses, and offices', 'text' => 'Browse a full Kyiv catalog that covers everyday homes, family spaces, and commercial listings.' ],
			[ 'icon' => '02', 'title' => 'Search by neighborhood', 'text' => 'Explore purpose, property type, district, and price without losing your place in the catalog.' ],
			[ 'icon' => '03', 'title' => 'Request a viewing', 'text' => 'Open any listing and contact the agency when you are ready to see it in person.' ],
			[ 'icon' => '04', 'title' => 'Talk to the agency', 'text' => 'Clear contact details make it easy to ask questions, compare options, and plan next steps.' ],
		];
		...
		$html .= '<span style="color: ' . esc_attr( $primary ) . '; font-size: 13px; font-weight: 900; text-transform: uppercase;">Kyiv brokerage</span>';
		$html .= '<h2 style="font-size: clamp(30px, 4vw, 48px); line-height: 1.08; margin: 8px 0 10px; color: ' . esc_attr( $style_tokens['heading'] ) . ';">Helping you search Kyiv with confidence</h2>';
		$html .= '<p style="color: ' . esc_attr( $muted ) . '; font-size: 16px; line-height: 1.65; margin: 0;">Compare listings, narrow the search, and request a viewing when the right property stands out.</p>';
		$html .= '</header>';
		...
		$html .= '<div>';
		$html .= '<span style="display: block; color: ' . esc_attr( $style_tokens['accent'] ) . '; font-size: 13px; font-weight: 900; text-transform: uppercase; margin-bottom: 8px;">Kyiv listings</span>';
		...
		$html .= '<p style="color: rgba(255,255,255,0.9); font-size: 15px; line-height: 1.55; margin: 0;">Browse current listings or request a viewing from any property page.</p>';
		$html .= '</div>';
