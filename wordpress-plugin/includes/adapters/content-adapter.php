...
public function plan( array $blueprint ): array {
		...
		foreach ( $items as $item ) {
			...
			if ( ! $post ) {
				$plan[] = [
					...
					'message' => "Create content item: {$post_type} в†’ {$title}",
				];
			...
			$target_state  = $this->get_target_post_state( $post_type, $item, $blueprint, true );

			if ( factory_is_post_user_modified( $post->ID, $current_state, $target_state ) ) {
				$plan[] = [
					'action'  => 'warning',
					'type'    => 'content',
					'entity'  => "{$post_type} РІвЂ вЂ™ {$title}",
					'message' => "Manual edits detected; this generated item will be preserved: {$post_type} РІвЂ вЂ™ {$title}",
			...
			if ( empty( $diff ) ) {
				$plan[] = [
					...
					'type'    => 'content',
					'entity'  => "{$post_type} в†’ {$title}",
					'message' => "Content up-to-date: {$post_type} в†’ {$title}",
				];
			...
			$plan[] = [
				...
				'type'    => 'content',
				'entity'  => "{$post_type} в†’ {$title}",
				'message' => "Update content item: {$post_type} в†’ {$title}",
				'diff'    => $diff,
			];
		}
	}
		...
		foreach ( $blueprint['content'] ?? [] as $post_type => $items ) {
			foreach ( $items as $item ) {
				...
				if ( ! $post ) {
					$results[] = [
						'status'  => 'error',
						'message' => "Missing content item: {$post_type} в†’ {$title}",
					];
				...
				$results[] = [
					'status'  => 'ok',
					'message' => "Content exists: {$post_type} в†’ {$title}",
				];
				...
				if ( factory_is_post_user_modified( $post->ID, $current_state, $target_state ) ) {
					$results[] = [
						'status'  => 'warning',
						'message' => "Manual edits preserved for generated property: {$post->post_title}",
					];
	...
	private function sync_post( string $post_type, array $item, array $blueprint ): ?array {
		...
		if ( factory_is_post_user_modified( $post->ID, $current_state, $target_state ) ) {
			factory_mark_post_user_modified( $post->ID );
			$this->log( "Manual edits detected; preserved: {$item['title']}" );
			...
			return $this->execution_item(
				...
				$post_type,
				$title,
				"Manual edits detected; preserved generated property: {$post->post_title}"
			);
		}
	...
	private function resolve_featured_image_source( string $post_type, array $item, array $blueprint, bool $advance_pool = true ): string {
		...
		return $this->resolve_asset_source_from_mapping( $mapping[ $property_type ] ?? '', $pool_key, $advance_pool );
	}

	private function resolve_asset_source_from_mapping( $mapping, string $pool_key, bool $advance_pool = true ): string {
	...
	private function resolve_local_asset_path( string $source ): string {
		...
		if ( defined( 'FACTORY_PLUGIN_DIR' ) ) {
			$candidates[] = rtrim( FACTORY_PLUGIN_DIR, '/\\' ) . '/' . $relative;

			if ( str_starts_with( $relative, 'blueprints/assets/' ) ) {
				$candidates[] = rtrim( FACTORY_PLUGIN_DIR, '/\\' ) . '/assets/' . substr( $relative, strlen( 'blueprints/assets/' ) );
			}
		}
	...
	private function get_current_post_state( WP_Post $post, array $item ): array {
		return [
			...
			'image'   => $this->get_current_featured_image_state( $post->ID ),
		];
	}

	private function get_target_post_state( string $post_type, array $item, array $blueprint, bool $advance_pool = false ): array {
		return [
			'title'   => $item['title'] ?? '',
			...
			'image'   => $this->get_target_featured_image_state( $post_type, $item, $blueprint, $advance_pool ),
		];
	}

	private function get_current_featured_image_state( int $post_id ): array {
		...
		$source = (string) get_post_meta( $attachment_id, '_factory_asset_source', true );
		$hash   = (string) get_post_meta( $attachment_id, '_factory_asset_hash', true );
	...
	private function get_target_featured_image_state( string $post_type, array $item, array $blueprint, bool $advance_pool ): array {
		$source = $this->resolve_featured_image_source( $post_type, $item, $blueprint, $advance_pool );
